"""Bundle installation helpers for nexus-n3-core plugin catalogs."""
...
def install_bundle(*, bundle_path: Path, activate: bool = True) -> int:
    """Install a built plugin bundle through the nexus-n3-core installer."""
    ...
    # excute the install file in nexus-n3-core/nexus_n3.plugins 
    cmd = [sys.executable, "-m", "nexus_n3.plugins", "install", str(bundle_path)]
...
def _pythonpath_entries(env: dict[str, str]) -> list[str]:
    ...
    # assumes the tooling is in the same directory as nexus-n3-core system
    # meaning alongside each other.
    nexus_n3_core_root = tooling_root.parent / "nexus-n3-core"
    entries = [str(nexus_n3_core_root)]
