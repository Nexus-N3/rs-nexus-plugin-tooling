"""Guided interactive nexus-n3-core system test workflow."""
    ...
    try:
        ...
        if current_plugin and current_plugin.plugin_type == "sensor":
            if current_plugin.sensor_type and _normalize(current_plugin.sensor_type) not in live_sensor_names:
                print("")
                print(f"Current sensor plugin '{current_plugin.display_name}' is not visible to the running nexus-n3-core.")
                print("Install its bundle first with `nx3-plugin install --bundle-path ...`, then rerun `nx3-plugin system-test`.")
                return 1
        if not sensors:
            print("No installed sensor plugins are visible to the running nexus-n3-core.")
            return 1
...
def _check_server_ready(client: SystemTestClient) -> dict:
    print("Checking nexus-n3-core readiness...")
    ...
    while time.monotonic() < deadline:
        ...
        try:
            ...
            if payload.get("msg") != "System Server Ready":
                raise RuntimeError("nexus-n3-core responded but is not ready for system testing.")
            print("nexus-n3-core is ready.")
            return event
        except TimeoutError as exc:
            last_error = exc
            continue
    raise RuntimeError(
        "Timed out waiting for nexus-n3-core readiness on the standard ZeroMQ gateway. "
        "Confirm nexus-n3-core is running and the zeromq gateway is active."
    ) from last_error
...
def _prompt_for_algorithm(
    ...
    if current_plugin and current_plugin.plugin_type == "algorithm":
        if _normalize(current_plugin.display_name) not in live_algorithms and _normalize(current_plugin.plugin_id) not in {
            ...
            print("")
            print(f"Current algorithm plugin '{current_plugin.display_name}' is not visible to the running nexus-n3-core.")
            print("Install its bundle first with `nx3-plugin install --bundle-path ...`, then rerun `nx3-plugin system-test`.")
    ...
    if not candidates:
        raise RuntimeError(
            f"Sensor '{sensor.display_name}' has no compatible installed algorithm visible to the running nexus-n3-core. "
