...
def _render_test_import(template: AlgorithmPluginTemplate) -> str:
    ...
    if template.include_intermediate or template.include_consolidation:
        imports.append("from rs_nexus_plugin_sdk import ExecutorBase")

    if template.include_intermediate:
        class_name = f'{template.class_name.removesuffix("Algorithm")}IntermediateExecutor'
        imports.append(f"from {template.package_name}.intermediate_executor import {class_name}")
        assertions.append(f"    assert issubclass({class_name}, ExecutorBase)")

    if template.include_consolidation:
        class_name = f'{template.class_name.removesuffix("Algorithm")}ConsolidationExecutor'
        imports.append(f"from {template.package_name}.consolidation_executor import {class_name}")
        assertions.append(f"    assert issubclass({class_name}, ExecutorBase)")
