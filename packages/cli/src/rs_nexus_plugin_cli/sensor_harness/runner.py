"""Generic source-mode sensor harness runner."""
...
from .sensor_manager import HarnessSensorManager
...
class CsvCaptureWriter:
    ...
    def write_event(self, event_name: str, payload) -> None:
        if event_name == "on_data":
            ...
            return
        self._error_handle.write(f"{time():.6f} {event_name} {serialize_payload(payload)}\n")
...
def parse_attributes(values: list[str]) -> dict[str, object]:
    """Parse repeated KEY=VALUE CLI attribute overrides."""
    parsed = {}
    for item in values:
...
def build_harness_config(args: argparse.Namespace) -> HarnessConfig:
    """Build a HarnessConfig from parsed CLI args."""
...
def serialize_payload(value):
    """Convert payloads into JSON-serializable values for harness logs."""
    if is_dataclass(value):
        data = asdict(value)
        sample_type = getattr(value, "sample_type", None)
        if sample_type is not None:
            data["sample_type"] = sample_type
        return {key: serialize_payload(item) for key, item in data.items()}
    if isinstance(value, SimpleNamespace):
        return {key: serialize_payload(item) for key, item in vars(value).items()}
...
async def run_sensor_test(config: HarnessConfig, *, output_dir: Path | None = None) -> int:
    """Execute the generic harness flow against a source-mode sensor plugin."""
